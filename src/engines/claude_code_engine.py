...
logger = logging.getLogger("hermes.claude_code")
...
def _build_cmd(project_dir: str, executable: str, instruction: str) -> str:
    if sys.platform == "win32":
        return f'cd /d "{project_dir}" && {executable} -p {_win_quote(instruction)}'
    else:
        import shlex
        return f'cd "{project_dir}" && {executable} -p {shlex.quote(instruction)}'


class ClaudeCodeEngine(ExecutionEngine):
    def __init__(self, executable: str = "claude", timeout: int = 600):
    ...
    async def execute(self, task: Task) -> ExecutionResult:
        ...
        try:
            ...
            logger.info(f"Executing in {task.workspace}: claude -p ...")
            ...
            try:
                ...
                process.kill()
                return ExecutionResult(success=False, error=f"Claude Code 执行超时 ({self.timeout}s)")
            ...
            return ExecutionResult(success=False, error=f"Claude Code 未安装或不在 PATH 中: {self.executable}")
    ...
    async def execute_with_progress(self, task: Task, progress_callback=None) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(success=False, error="Claude Code 未安装或不在 PATH 中")
        except Exception as e:
            return ExecutionResult(success=False, error=f"执行异常: {e}")
        finally:
            self._processes.pop(task.task_id, None)
    ...
    def supported_intents(self) -> list[str]:
        return ["code_review", "explain", "code_modification", "bug_fix"]
